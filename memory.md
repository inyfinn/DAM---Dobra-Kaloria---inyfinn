...
- **Skojarzone materiały (viz modal + media preview)**: ten sam filtr co Branding — `isSourceLikeAsset()` + `passesMarketingAssocMaterial()` w `dam-media-preview.js` (zero AI/PSD/PSB/PDF/vector/document/source). Ładowanie = `assocPaneSkeletonHtml()` + `DamLoader.start("Skojarzenia…")`, **bez** gołego `<p>Ładowanie…</p>` w `#damVizModalAssoc`.
