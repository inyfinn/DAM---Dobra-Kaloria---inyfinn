...
(function () {
  ...
  function loadIntegrations() {
    var box = document.getElementById("damIntegrationsList");
    if (!box) return;
    ...
    function renderOauth(data) {
      ...
      var oauthCards = keys.map(function (k) {
        ...
        return cardHtml({
          id: k,
          label: p.label || k,
          desc: p.hint || "OAuth firmowy - tokeny lokalnie (Fernet).",
          icon: ic.icon,
          iconClass: ic.cls,
          status: st,
          statusLabel: stLabel,
      ...
      var extras = EXTRA_INTEGRATIONS.map(function (x) {
        return cardHtml({
          id: x.id,
          label: x.label,
          desc: x.desc,
      ...
      box.innerHTML =
        ...
        oauthCards.concat([synCard]).concat(extras).join("") +
        "</div>";
    ...
    fetch(bridge() + "/integrations/status")
      .then(function (r) {
        return r.json();
      })
      .then(renderOauth)
      .catch(function () {
        box.innerHTML =
          ...
          cardHtml({
            id: "offline",
            label: "Integracje OAuth",
            desc: "Bridge offline - status Asana / Microsoft niedostępny.",
            icon: "uil-wifi-slash",
            status: "wait",
            statusLabel: "Most lokalny offline",
          }) +
